...
type SortField = 'title' | 'artist' | 'bpm' | 'duration' | 'filesize' | 'rating' | 'bitrate'
...
export default function Tracklist({
  ...
  // Clear scanned list when selected playlist changes
  useEffect(() => {
    ...
    const scanMissingBpm = async () => {
      ...
      try {
        while (true) {
          ...
          try {
            console.log(`Scanning BPM for track ${trackToScan.title}...`)
            ...
            window.api.logError(`Failed to calculate BPM for ${trackToScan.title}: ${e?.message || e} (Path: ${trackToScan.filepath})`)
          } finally {
              ...
              return (
                <tr 
                  ...
                  className={`hover:bg-zinc-900/30 group transition-colors ${
                    isPlaceholder ? 'opacity-60 cursor-not-allowed select-none' : 'cursor-grab active:cursor-grabbing'
                  } ${
                        ...
                        return (
                          <button
                            ...
                            className={`transition-colors p-0.5 ${
                              isPlaceholder ? 'text-zinc-800 cursor-not-allowed' : 'text-zinc-600 hover:text-amber-500'
                            }`}
                          >
                            <Star 
                              className={`h-3.5 w-3.5 ${
                                isFilled ? 'fill-amber-500 text-amber-500' : isPlaceholder ? 'text-zinc-800' : 'text-zinc-700 hover:text-amber-500'
                              }`} 
                            />
                          </button>
                        )
